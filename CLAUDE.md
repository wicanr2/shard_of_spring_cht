...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(`75`/`80`/`81`/`83`/`87`/`88`/`89`:記錄長度 94/90、讀寫端都有、**39 個位移清點完成、8 個有語意**。**缺:語意覆蓋率**)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | 進行中(`72`–`74`、`82`/`83`/`90`:格式定案、**怪物十欄全部有語意**、法術四欄、物品欄1–5。**缺:物品欄6、法術效果類別**)|
