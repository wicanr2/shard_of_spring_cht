...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(`75`/`80`/`81`/`83`/`87`/`88`:記錄長度 94/90、`GROUPS.DAT` 15 個位移、`CHARS.DAT` **10 個位置有語意**、種族/職業碼表。**缺:其餘位移、`CHARUTIL` 寫入端**)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | 進行中(`72`–`74`、`82`/`83`:格式定案、怪物十欄定案九欄。**缺:怪物欄9、物品欄6、法術效果類別**)|
