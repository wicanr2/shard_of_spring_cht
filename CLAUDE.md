...
> **A、B、C、F、G、H、I、K、L 已 RE-DONE**(`docs/re/67` §3、`68` §3、`66` §4、`54` §4、`60` §5、`49` §4、`61` §4、`62` §5、`71` §5 逐條核對)。
...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(`75`/`80`/`81`:記錄長度 94/90 從 `OPEN` 讀出、`GROUPS.DAT` 15 個位移、`CHARS.DAT` 14 個位移的去處。**缺:位移語意、`CHARUTIL` 寫入端**)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | 進行中(`72`–`74`、`82`:格式定案、法術四欄、物品五欄、**怪物十欄定案七欄**。**缺:怪物欄4/欄9、物品欄6、法術效果類別**)|
...
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | 進行中(`76` 命中、`79` 傷害、`81` 屬性 1–14 的來源。**缺:先攻、逃跑、屬性 16/17 的算法**)|
