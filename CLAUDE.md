...
1. **Remake** — 重寫一套可維護、可跨平台執行的引擎。技術棧已定案：
   **Go + Ebitengine**（2026-08-14，見 `docs/spec/03-engine-plan.md` §1）。
...
> ✅ **這道閘門已經通過**（2026-08-14）：§2.2 看板 A–L 十二個子系統全部 RE-DONE，
> 規格標 READY，技術棧定案。**現在可以寫引擎程式碼。**
> 本節保留為判準——之後若有新的子系統或格式要解，同一套四項條件仍然適用。
...
| 3 | **有一份獨立資料能互相印證**（另一個檔案、另一支模組、手冊、攻略；DOSBox 實跑也算，但**不必要**） | 再加一條「自己驗自己」的單元測試 |
...
> **十二個子系統(A–L)全部 RE-DONE**(`122` §4 索引;J 曾在 `97` 宣告、`102` 撤回、`103` 重新收斂)。
> ⚠ 各篇收斂筆記都附「明列剩餘的不確定」,**RE-DONE 不等於零疑問**。(`docs/re/67` §3、`68` §3、`66` §4、`54` §4、`60` §5、`49` §4、`61` §4、`62` §5、`71` §5、`91` §3、`97` §4 逐條核對)。
> ⚠ G 曾在 `57` §5 判定、`59` §5 因範圍擴大而收回、`60` §5 重新判定 —— 過程留在紀錄裡。
> 任何一列變成 RE-DONE 之前，對應的 remake 程式碼一行都不能寫。

| 子系統 | 主要輸入 | 狀態 |
|---|---|---|
| A. 執行檔架構與模組轉交 | `START/MENU/TOWN/CMBT/CAMP/MAZEMOVE/WRLDMOVE/CHARUTIL.EXE` | ✅ **RE-DONE**(`01`–`06`/`09`/`43`/`47`/`67`:stub 全流程、`bz` 標頭與重定位、程式碼起點、**模組轉交 = `retf` 進 `節區:0x30`(`bm` 為 `0x3A`)**、COMMON 區。四項條件核對見 `67` §3)|
| B. 執行期模組的呼叫介面 | `BRUN30.EXE` | ✅ **RE-DONE**(`06`/`40`/`41`/`44`/`45`/`64`/`68`:INT ABI、`3Dh` 自我改寫、**三張表容量 105/165/≤204**、`3D:00` 繫結到 `USERLIB`、DS 基底、MS BASIC 5.60。四項條件核對見 `68` §3)|
| C. 原生輔助程式庫 | **`USERLIB.EXE`**(另三支已改歸 A)| ✅ **RE-DONE**(`30`/`63`–`66`:11/11 載入、字串表 60 段、`3D:00` 呼叫機制、匯出表 63 槽、六個槽語意定案(訊息框/視窗/存檔/狀態列/死亡/結局)。四項條件核對見 `66` §4)|
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | **RE-DONE**(`122` §2 逐條核對;兩個記錄的欄位全部有語意)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | **RE-DONE**(`91` §3 逐條核對;22 個欄位全部有語意)|
...
| I. 法術效果表 | `FIRESTRM/HAILSTRM/WINDSTRM.BIN`(`DE*EFF` 已改歸 G)| ✅ **RE-DONE**(`61`:三張圖是 17×17 圖塊但**遊戲從不載入**(窮舉搜尋 + 正對照);法術規則屬 E、戰鬥呈現屬 J。四項條件核對見 `61` §4)|
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | **RE-DONE**(`103` §4 逐條核對;逃跑 = 全隊離場)|
| K. 輸入語意（**不含 BIOS 層**） | 哪些鍵在哪個畫面有效、對應什麼動作 | **RE-DONE**(`71` §5 逐條核對;小鍵盤轉譯層 + 七支模組比對鏈 + 五個 `USERLIB` 槽)|
| L. 中文化落點盤點 | 全部(見 §7)| ✅ **RE-DONE**(`62` + `63` §4 訂正:1,476 段 / 34,499 B;45% 可變長、55% 只能等長)|

`CONFIG.SOS`、`STARTUP.BIN`、`BORDER.BIN` 這類純顯示設定，依 §1.2 **只需知道效果**，
不單獨列為 RE 項目。

### 2.3 閘門前允許寫的程式碼（閘門已通過，本節現在只約束「新開的未解項目」）
...
⚠ 對**尚未 RE-DONE 的新項目**，「先寫個 prototype 試試看」仍然不在允許範圍內 ——
那正是產生「通過測試但其實是錯的」實作的路徑。
...
1. **人親自實測原版**
2. **IDA 反組譯**（本專案的主要來源，十二個子系統都是這樣解的）
3. 官方手冊 / 社群攻略

**低層級不可推翻高層級。** 但這條的重點是**第 1 級** ——
被真人實測打臉的推論就是錯的，不論反組譯看起來多合理。

⚠ 套手冊之前先問「這一頁講的是哪個平台」。Shard of Spring 有
Apple II / C64 / DOS 三個版本，手冊描述的規則不保證等於 DOS 版行為。

### DOSBox 是輔助驗證，不是卡控

`tools/dosbox_run.sh` 可用（`docs/re/32`）。**它不是任何結論的必要條件** ——
§2.1 條件 3 要的「一份獨立資料」，跨檔案、跨模組、手冊、攻略都算數，
十二個子系統的收斂沒有一項靠它。

實跑可以一路到世界地圖（`docs/re/139`）：開機的手冊查表題按 Enter 就過，
`L` → `5` 載入 PARTY #5，狀態欄與 `GROUPS.DAT` 第 5 筆逐欄吻合。
⛔ 不從程式裡取出預存答案來自動通過檢查（§1「不做的事」）——
這條約束與「題目擋不擋得住」無關，仍然有效。
...
顯示層已定案:**畫布 1024×768,美術 4× 整數放大,文字層用 TTF 走原生解析度**
（`docs/spec/04-display-layout.md`，標 READY）。中英混排寬度、避頭尾、
覆蓋層與側欄的欄寬預算都在那份規格裡。
...
**RE 階段已結束** —— §2.2 看板 A–L 十二個子系統全部 RE-DONE，
規格收攏並標 READY（`docs/formats/01`–`07`、`docs/spec/01`–`03`），
技術棧定案 **Go + Ebitengine**。**§2 的閘門全開，可以寫引擎程式碼。**

現在的下一步在 `docs/spec/03-engine-plan.md` 的 M0–M7，不在本檔。
現況一覽以 `CONTEXT.md` §2 為準。
