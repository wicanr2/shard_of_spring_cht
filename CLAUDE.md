...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(記錄長度 94/90、讀寫端、39 個位移清點、**9 個位置有語意**。**缺:語意覆蓋率**)|
...
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | 進行中(`76` 命中、`79` 傷害、`83` 死亡、`84`/`86` 移動、`92` 目標與旗標;**19 個屬性裡 12 個有語意**。**缺:先攻、逃跑**)|
