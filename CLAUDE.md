...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(`75`/`80`:記錄長度 94/90 從 `OPEN` 讀出、筆數 25/5 整除、`GROUPS.DAT` 15 個欄位位移全列、裝備欄語意。**缺:欄位語意覆蓋率 —— 靜態工作,不卡外部條件**)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | 進行中(`72`–`76`:格式定案、法術四欄、物品欄4/欄5 的雙重身分確認。**缺:怪物另九欄、物品欄6**)|
...
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | 進行中(`76` 命中公式、`77` 算術 API、`79` **傷害公式**。**缺:先攻、逃跑、屬性17 語意、兩個係數來源**)|
