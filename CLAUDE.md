...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(`75`:25×94 整除、後 20 筆為空、位移 34/36 = 裝備背包格號、54+2i = 背包物品。**缺:其餘欄位與寫入端**)|
| E. 規則資料表 | `MONSTERS.DAT` / `SPELLS.DAT` / `ITEMS.DAT` 的格式與欄位語意 | 進行中(`72` 格式定案、`73` 怪物欄去處、`74` 法術四欄 + 物品五欄語意確認。**缺:怪物十欄語意、物品欄6 與分類依據**)|
...
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | 進行中(`73`/`75`:15 欄二維陣列、屬性 4/5 = 武器/防具、隊伍與怪物同一條初始化路徑。**缺:公式**)|
