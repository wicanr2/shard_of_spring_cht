...
| D. 角色／隊伍資料與存檔 | `CHARS.DAT` / `GROUPS.DAT` 的欄位與存檔流程 | 進行中(記錄長度 94/90、讀寫端、39 個位移清點、**23 個位置有語意**(`94`)。**缺:其餘位移**)|
...
| J. 戰鬥規則 | 命中／傷害／先攻／逃跑的判定 | 進行中(`76` 命中、`79` 傷害、`83` 死亡、`93` 先攻、`96` 朝向與行動點數;**19 個屬性裡 15 個有語意**。**缺:逃跑的證據等級**)|
