...
| **派工表是薄包裝(B)** | `docs/re/35`,多個進入點共用一支實作,參數寫在 `call` 之後 |
...
| 每個派工索引是一支獨立常式(`docs/re/12`–`14` 的前提) | **多數是薄包裝** —— 相鄰索引常只是同一支實作的不同參數(`call` 後接 2 bytes,被呼叫端 `pop`+`lods` 取走)。解表策略應改為「找共用實作,一次解一族」。`docs/re/35` |
