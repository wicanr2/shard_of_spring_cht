...
| 項目 | 卡在哪 |
|---|---|
| **引擎實作 M3** | M0 ✅、M1 ✅、M2 ✅(世界地圖走得動,座標經畫面驗證)。M3 = 角色與存檔 |
| ⛔ **世界地形的圖塊映射** | 58% 的格子(主要是海洋)沒有已知圖塊來源,映射不是單一線性偏移(`docs/re/128`)。畫面上是一整片洋紅佔位符。要讀 `WRLDMOVE` 的繪製迴圈才解得開 |
...
| [`128-wrlditem-tile-bias.md`](docs/re/128-wrlditem-tile-bias.md) | **圖塊偏移 +14**,且映射非線性 | 排除法只排掉一個候選不算證明;用兩側已知語意的錨點對,7/7。⚠ 海洋等 58% 無來源 |
...
| `WRLDITEM.PIC` 的圖塊偏移 = +11(`docs/formats/05` §3,我用排除法推的)| **= +14**(`128`)。用「整片海洋不可能是磚牆」排掉一個候選之後,我就停在剩下的第一個 —— 但候選空間是所有整數偏移,排掉一個還剩無限多 | **排除法要排到只剩一個才算數。「A 不可能」推不出「B 成立」**,除非已證明候選只有 A 和 B。正解是拿**兩側都已知語意的錨點**去對(城鎮 13 處、地城入口 11 處),7/7 |
