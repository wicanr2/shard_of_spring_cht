...
| **模組內文本的翻譯(F1)已完成** | 清冊 **828 段**(`tools/dgroup_strings.py` 掃 11 支模組):`ui` **381 段全部譯完**、`data` 327 段(數字/檔名/單字元)、`na-*` 120 段(換磁片/印表機/除錯,remake 不做)。⚠ **剩下的是接線不是翻譯**:引擎畫面用的是實作時自己寫的中文(`tools/check_ui_language.py` 量到 0 條英文),把它換成原版措辭是 worklist 的 **F3**(371/381 已接,其餘 10 段已裁定不接)|
...
剩下的是把那些措辭**接回引擎**(worklist 的 F3,**371/381**(其餘 10 段已裁定不接)),
