...
| **模組內文本的翻譯(F1)已完成** | 清冊 **828 段**(`tools/dgroup_strings.py` 掃 11 支模組):`ui` **381 段全部譯完**、`data` 327 段(數字/檔名/單字元)、`na-*` 120 段(換磁片/印表機/除錯,remake 不做)。⚠ **剩下的是接線不是翻譯**:引擎畫面用的是實作時自己寫的中文(`tools/check_ui_language.py` 量到 0 條英文),把它換成原版措辭是 worklist 的 **F3**(330/381 已接)|
...
剩下的是把那些措辭**接回引擎**(worklist 的 F3,**330/381**),
難點不是長度(remake 沒有等長限制)而是還不知道每條字串的顯示位置。
⛔ 未接的 51 段不等於「還沒做」——`docs/spec/19` §6.1 列了三種成因,
