...
| **DOSBox oracle** | `tools/dosbox_run.sh` 未建。開機的手冊查詢題目由負責人提供(見 §5「不碰的東西」)。⚠ `docs/spec/00-index.md` 有一項「實作前必讀」需要實跑裁決 |
...
唯一還缺的外部條件是 **DOSBox oracle**(`docs/spec/00-index.md` 的五項
「實作前必讀」裡有一項需要實跑裁決),它擋的是驗收不是動工。
