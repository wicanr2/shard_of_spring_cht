...
| **DOSBox oracle** | `tools/dosbox_run.sh` **已建**(`docs/re/32`),但實跑停在開機的手冊查表題,進不了遊戲畫面。要往下走只有一條合法途徑:**由持有原版的人依手冊作答**(不從程式取出預存答案,`docs/re/32` §3.1)|
...
**DOSBox oracle 卡在開機的手冊查表題**,需要持有原版的人依手冊作答。
⚠ 它擋的不是動工,也不是十二個 RE-DONE 的任何一項
(那些的條件 3 都由跨檔案/跨模組的獨立來源滿足,不靠 DOSBox);
擋的是 `docs/spec/00-index.md`「標 READY 不等於零疑問」列的那批洞 ——
傷害公式的兩個係數、戰鬥屬性 14/18、法術效果類別 3、`CHARS.DAT` 位移 1、
先攻是否每回合重排。那幾項靜態已經到頂,要實跑才裁決得了。
