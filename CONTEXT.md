...
| **施法的三段管線** | `docs/re/173`,選目標(類別 1 跳過)→ 共同段(擲 **d100** `ds:977E`、算 `欄4 ÷ 欄5`)→ **`INT 3F:5E` 帶內嵌 13 項表**依效果類別分派。13 = 類別數,而**類別 3–7 共用一個目標**,與 `171` 的 `ds:9B32` 鏈接得上。⚠ **表的落點有三項在指令中間 → 解析沒驗證完**,能用的只有結構不是位址 |
