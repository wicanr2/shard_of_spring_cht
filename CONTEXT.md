...
| **遭遇的怪物怎麼挑** | `docs/re/169`,由**區域編號** `ds:3656` 決定:隨機挑一隻,`|難度階級 − 區域| > 1` 就重擲(`neg` 是讀到的 → **絕對值**)。世界地圖分三區(x=70 / y=60 切),迷宮逐座指定,DG5 還按樓層座標再分兩區。⚠ 原版重擲**沒有上限**,湊不到會當掉。順帶:**`ds:3534` = 當前迷宮編號、99 = 野外**,關掉 `166` §2 的但書 |
