...
| **端到端迴歸網(T1/T2)** | `engine/integration_test.go`,讀**版控裡的真資產**無頭跑完一輪(讀檔 → 走路 → 戰鬥 → 進城 → 買東西 → 存檔 → 讀回)+ 同種子跑兩次比指紋。⚠ **涵蓋範圍有邊界**:`Update()` 只有外殼/覆蓋層/名冊那幾段走 `g.testKeys` 接縫,世界地圖·城鎮·戰場是**直呼 handler**,派工那幾行沒人看著(`docs/spec/14` §8)|
