...
> 🎨 **手繪冒險書現代主題已完成（2026-08-20）**：專案負責人確認
> 視覺與配樂分開切換、現代美術採手繪冒險書，並允許生成式 AI 初稿加人工指定
> 後製。`F6` 已接成「原版／手繪冒險書」，`F5` 配樂保持獨立；title／logo、
> 世界與迷宮 tileset、玩家與怪物 sprite、戰場圖塊及羊皮紙 UI 已接線。
> 規格與權利紀錄見 `docs/spec/24-modern-storybook-theme.md`、`art/modern/README.md`。
> 兩套字型完整測試已通過；README 已加入四張新版截圖，57 秒推廣片加入真實
> `F6` 切換段；`v0.5.1` 另修正角色比例、怪物 atlas 分割與土黃色亮度，
> 三平台完整包與 SHA-256 清冊已在本機建置及驗證。
> 2026-08-21 後續美術決策採用深褐黑 `maze-umber` 主視野底色，並為現代世界
> 的海洋、海岸、草原與森林加入四向鄰接自動拼接；原版索引、規則與存檔不變。
...
> 🆕 **戰場地形已解並接線**(2026-08-20,[`227`](docs/re/227-fastcmbt-is-nine-get-arrays.md)):
> 來源是隊伍周圍的 3×3 地圖格,一格攤成 5×5;原版跑兩趟而**第二趟覆寫第一趟**,
> 所以只有海洋(→ 水面)與世界值 10(→ 岩漿)會長出地形。`v0.3.0` 之後的改動。
>
> 📦 **最新本機完整包:`v0.5.2`(2026-08-21)** —— 三平台(Linux AppImage / Windows zip /
> macOS universal)。`v0.3.0` 是三輪 QA 之後的第一版(遭遇改成一群怪、怪物會施法、
> 規則層四條訂正、畫面照原版重排),`v0.3.1` 補戰場地形,`v0.3.2` 補戰鬥音效,
> `v0.4.0` 收商店賣出(重製版增補)、夜視與怪物知識兩項技能、移轉術、
> 怪物法術第二回合的訂正、整頁鍵盤對照；`v0.5.0` 加入可獨立切換的手繪冒險書
> 視覺主題；`v0.5.1` 修正角色比例、怪物 atlas 殘影與介面亮度；`v0.5.2`
> 加入深褐黑迷宮暗部與現代世界自然地形四向自動拼接。
> `v0.5.2` 已於 2026-08-21 建立 private GitHub Release，包含三平台封包、
> SHA-256 清冊與最終推廣片；tag 指向 `cd7fef8`。
> ⚠ **`v0.3.1` 起資產要重轉**(遭遇表與戰場圖塊是新的);`v0.4.0` 存檔與資產相容。
...
| 項目 | 卡在哪 |
|---|---|
| **經驗值已定案** | 存在 `CHARS.DAT` **位移 90–93,MBF 單精度**(`docs/re/150`),引擎讀寫原版記錄。**每一場給多少也讀出來了**(`docs/re/152`):每人所得 = `INT(九個怪物槽的屬性 19 總和 ÷ 有資格人數)`,屬性 19 來自 `MONSTERS.DAT` 欄 8。⚠ 累加迴圈**不篩死活** —— 逃走的怪照樣給經驗,而多數戰鬥打到全滅才結束,所以這個差別在畫面上看不出來 |
...
| **原版有音效(新解)** | [`228`](docs/re/228-combat-sound-effects.md):`CMBT` 有一支「兩個字母的代碼 → 一串 `SOUND`」的常式,**20 個呼叫端**、七個代碼(`BP`/`HT`/`HK`/`BR`/`DD`/`PS`/`FL`)。`S` 鍵切的是**音效**不是音樂(`ds:9306`)。引擎接了四個有證據的觸發點(命中/劈砍/死亡/施法),`BP`/`FL`/`BR` **資料照抄但不接**。⚠ 缺口的成因值得記:[`216`](docs/re/216-the-game-has-two-tunes.md) 掃的是 `PLAY` 的樂譜字串,而音效走 `SOUND` —— **掃 `PLAY` 結構上就看不到它們**,那一篇自己也標了「沒查」,但「全遊戲只有兩首曲子」讀起來很像「其他時候是安靜的」 |

| **效果缺口盤點(2026-08-20)** | `docs/spec/14` §13,逐個消費端 `grep` 出來的。三項技能買了沒有效果 —— 夜視與怪物知識**當天就解掉並接線**(`docs/re/229`),只剩說服(降價未解,標在商店畫面上)。⚠ 巫師 8「物品知識」是**原版自己的死技能**(`re/167` §4),不是缺口。另外:移轉術(效果類別 13)、音效 `BP`/`FL`/`BR`、怪物 AI 的 `ds:7340` 與系別 2/5、小鍵盤九宮格圖。執行順序在 §13.6 |
| **商店賣出(增補,非還原)** | 專案負責人裁定 2026-08-20(`docs/spec/14` §13.7):收購價 = 該店售價 × 65%,只有有販售範圍的店收、只收自己賣的範圍、裝備中的不能賣。⚠ **原版沒有這個功能**(`re/142` §4),所以它與場景配樂同一個地位 —— **要在畫面上標明是重製版增補** |

| **夜視與怪物知識已解** | [`229`](docs/re/229-night-vision-and-monster-lore.md):**夜視** = 火把滅掉之後的能見度 2→3(進迷宮時重算,要**戰士**且有旗標);**怪物知識** = `?` 面板整組數值的條件。順帶三條:`?` 與 `/` 都開面板、「目標>」看的是**行動者自己**會不會策略(手冊 p.35 說「隊上有人會」,程式碼不是)、`Magical:` 判的是屬性 7 > 0(交叉印證屬性 7 = 法力)。⚠ 獨立印證:出貨 PARTY #5 的位移 61 就是程式碼寫的預設值 2 |

| **移轉術已解(33/33 法術全部有效果)** | [`230`](docs/re/230-transference-transfers-spell-points.md):效果類別 **13 與類別 5 共用同一支常式**,差別只有記錄位移 —— 5 動 28(生命值)、13 動 **32(法力)**,上限取「位移 − 2」。欄4=欄5=3 → **1:1 轉移**,另有一道 `min(量, 投入)` 擋「憑空生法力」。順帶讀出三道夾住(下限 **3**、最大值、硬上限 **255**)|

| **音效 `BP`/`FL` 查完了,不接** | [`228`](docs/re/228-combat-sound-effects.md) §6:十個呼叫端全部讀過。五個 `BP` 是游標移動那一組(正常路徑**跳過** BP)、三個的閘門是「行動點數 < 3」、`FL` 在施法路徑上。⛔ **兩種讀法都相容而接到的位置相反**(「做不到」vs「做完了」),所以不接。順帶認出 **`ds:956C` = 行動點數**(−1/−3/−2 與 0–3 的比較,對得上成本表)。✅ 剩下的問題已縮小成**一次 DOSBox 實跑**就能回答 |

| **怪物 AI 兩格關掉(含一個沒有症狀的錯)** | [`231`](docs/re/231-monster-spell-round-two-falls-back.md):①`ds:7340` **早就解過了** —— 它是 `SPELLS.DAT` 欄3(效果類別),`== 1` 就是「群體傷害」;過期的是 `re/170` §6 那張表。②系別 2/5 第二回合起 `0x155AA` 的 `jmp` 落在 **`0x155B0`**(第一回合的擲骰入口)—— **照樣施法**。⚠ 引擎先前回 0,那兩個系別的怪**從第二回合起再也不施法**,而畫面完全正常 |
...
| **oracle 可以錄聲音了,但 `BP` 那一題卡在模擬器層** | `SOUND=1 tools/dosbox_run.sh` 掛一份改兩行的 entrypoint(`pcspeaker=true` + `nosound=false` + `SDL_AUDIODRIVER=dummy`)。⚠ **兩個設定要一起改**。⚠ 錄到的「聲音」逐取樣讀出來是**常數 DC**(−4999/−5000,從不過零)—— 真機上那是無聲,而先前算的「400 Hz」是把過零率算在一段從不過零的訊號上。**遊戲有去驅動喇叭,DOSBox 沒把它算成音**(`docs/re/228` §6.7)。⚠ 順帶訂正 §2 的方向:**音效預設是開的**(閘門是「等於 −1 才跳過」,初值 1)|
...
| 原斷言 | 實際 | 錯誤形狀 |
|---|---|---|
| 「`ds:9306` 不是 −1 就整支跳過」(`docs/re/228` §2)| **方向反了**:`jnz` 去的是繼續發聲那一邊,**等於 −1 才跳過**,初值 1 = 音效**開** | **`jnz` 之後的 fall-through 與跳躍目標讀顛倒** —— 兩種讀法都自洽,而且都能寫出一句通順的註解 |
| 「錄到峰值 5000 = 錄到聲音」(`docs/re/228` §6.5 初稿)| 是**常數 DC**,從不過零 —— 真機上無聲 | **從聚合統計(峰值、過零率)推波形** —— 順序反了,先看波形再算頻率 |
| 「系別 2/5 第二回合起沒有分支 → 不施法」(`internal/rules.MonsterSpell`)| `jmp` 落在**第一回合的擲骰入口**,照樣施法(`docs/re/231` §2)| **「沒有那一條分支」與「不做那件事」是兩回事** —— 讀到 `cmp` 鏈最後一個 `else` 要去看那條 `jmp` 落在哪裡 |
| 「`ds:7340` 旗標陣列未解」(`docs/re/170` §6)| 它是 `SPELLS.DAT` 欄3,`docs/re/171` §1 早就讀出來了 | **「未解」清單沒有跟著解出來的那一篇收掉** —— 成本是下一輪有人真的回去重查一次 |
| 「`ds:6F10` 是**補給品**上限」(`docs/re/218` §1)| 生命值與法力寫回記錄時夾的是**同一個變數**(`docs/re/230` §3)—— 它是通用的 255 上限 | **在第一個看到它的情境裡替變數命名** —— 那一輪是從打獵那條路徑進去的,名字就跟著那條路徑走了 |
| 「隊上有人會策略就看得到怪物在追誰」(手冊 p.35,`docs/re/186` §2)| 程式碼看的是**當下行動者自己**(`docs/re/229` §3.1)| **手冊講效果、不講條件的粒度** —— 「隊上有人」與「這個人」在多數情況下結果相同,差別只在換人行動的那一刻 |
| 「`?` 面板對怪物就是印那一組數值」(`engine/inspect_scene.go`)| 整組數值是**巫師 + 怪物知識**才看得到的第三級(`docs/re/229` §3.2)| **一張截圖只證明它拍到的那一格** —— `i1-inspect.png` 拍的是隊員,怪物那一側沒測到 |
| 「原版只有兩首曲子,其餘時候是安靜的」(`docs/re/216` 的讀法)| **戰鬥有一整套音效**,七個代碼 20 個呼叫端(`docs/re/228`)| **清點的分類方式決定了清點的邊界** —— 掃的是 `PLAY` 的樂譜字串,而音效是 `SOUND`。原篇誠實標了「沒查」,但標題與結論讀起來像「沒有」 |
| 「`PS` 是 8 圈、`FL` 是 4 圈」(本輪初稿)| **7 圈與 3 圈**(`cmp ax,7 / jle`、`cmp ax,3 / jle`)| **從迴圈本體數 `SOUND` 的個數,不是從 `cmp` 讀次數** |
...
| 「`INT 3F:55` 是開檔」(`132` §2)、後來改成「兩數相加」(`38` §1)| **兩個都不對:是字串相加 `A$ + B$`**(`38` §1 訂正)。前六條指令把兩個**長度**加起來,`call sub_1A08B` **之後**還有兩次 `rep movsw` 把內容搬過去 —— 讀到那個 `call` 就停,看到的正好是一個像整數加法的形狀。實際用途是組檔名:`<路徑> + "rndmonst.bin"` | **一支 API 在 `call` 之後還有指令時,那些指令往往才是它真正在做的事。** 兩次命名都錯在「看形狀」:第一次看呼叫端的形狀(旁邊有檔名 → 開檔),第二次看本體的前半(有 `add` → 加法)。判準:讀到 `retf` 再命名 |
...
| 「遭遇比對的是 `MONSTERS.DAT` 欄 9(難度階級)」(`169` §1 第 3 項,標證據充分)| **是 `RNDMONST.BIN` 的欄 0**(`225` §5)。那張陣列是 `CMBT` 用 `VARPTR` 把位址交給原生常式去填的,填的是這個檔 | **統計佐證分不開兩個值域相同的讀法。** `169` §1.1 的四項檢查(七區都有候選、難度單調、值域、語意)對**兩個**讀法都成立 —— 真正分得開的是**組成**(一場是一群、會混相鄰階級),而那件事當時沒有被拿來當判準 |
| 「系別 1 的攻擊法術有 3 張」(`170` §3,由「只挑攻擊法術」推的)| **是 2 張**(`226` §1):`INT 3F:AD` 的內嵌位元組加在**浮點指數**上,`01` = ×2。連帶:第一回合的群體傷害重擲只對系別 4 生效 | **「由同一個設計推」得到的數字要標成假設,而且要記得它擋著什麼。** 那句推論擋的是一條 API 的語意,而那條 API 只要 dump 一次本體就讀得出來 |
| 「系別 3 是 1…2 的擲骰」(引擎 `rules.SpellFamilyAttacks`)| **不擲骰**,`mov ds:9B14h, 2` 是常數 → 恆為法術 9(`226` §1)| **常數與「面數為 2 的擲骰」在表格裡長得一樣。** 原文 `170` §2 寫的就是「常數 2」,是搬進程式碼時被讀成了張數 |
| 「`FIRESTRM`/`HAILSTRM`/`WINDSTRM.BIN` 遊戲從不載入」(`61` §2,做過正對照的檔名搜尋)| **內容有被載入**:`FASTCMBT.BIN` 是那**九個 98-byte 檔的打包本**(逐位元組相同),`CMBT` 一次 `BLOAD` 進 `ds:7B20`,施法時取第 6/7/8 槽來畫(`227` §1)| **檔名搜尋回答的是「誰開了這個檔」,不是「這份內容有沒有被用」。** 那一節的方法沒錯(正對照也做了),錯在**把答案套到另一個問題上** —— 同一份像素換一個檔名就進來了 |
| 「戰場地形是從周圍的地圖格鋪出來的」(`227` §2 初稿,只讀了產生器)| **未證實**:合成地形實測(把工作複本的地圖改成上兩列森林、下一列山)得到**整片全黑**,與區域 6 那場「海洋 → 水面」矛盾(`227` §2)| **讀到一段算式不等於知道它的輸入從哪來。** `ds:3740` 由**送出戰鬥的模組**填,填什麼沒讀 —— 在那之前,那段產生器可能根本沒被走到 |
