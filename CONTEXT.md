...
| **效果缺口盤點(2026-08-20)** | `docs/spec/14` §13,逐個消費端 `grep` 出來的。**三項技能買了沒有效果**:夜視(戰士5)、怪物知識(巫師9)—— 兩者連「未解」都沒標在畫面上;說服(戰士10)有標。⚠ 巫師 8「物品知識」是**原版自己的死技能**(`re/167` §4),不是缺口。另外:移轉術(效果類別 13)、音效 `BP`/`FL`/`BR`、怪物 AI 的 `ds:7340` 與系別 2/5、小鍵盤九宮格圖。執行順序在 §13.6 |
