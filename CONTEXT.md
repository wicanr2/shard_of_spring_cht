...
| **角色創造流程** | `docs/re/143`,種族修正確實生效(5/5)、初始生命 = 加完修正的體質、技能點數 = 智力;骰法未解但 **3d6 已排除** |
...
| `GROUPS.DAT` 時鐘的四個上限「未解且**不可從這些數字推**」(`formats/02`)| 手冊 p.32 寫明 22 月 / 34 天 / 26 小時,與 34、26 精確相符(`140` §4)| **「不可推」講的是「不能從這四個數字自己推」,不是「沒有別的來源」。** 寫下這種封鎖語句時要順手問一句:哪一種來源能解開它? |
