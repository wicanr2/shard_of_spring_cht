...
| 項目 | 卡在哪 |
|---|---|
| **解 `ds:74h` 累積的三個參數是什麼** | 位址 `0xC26`–`0xC30`,取出時做位元組序反轉。用 xref 掃該範圍找其他讀取端 |
...
| [`14-brun-unlock-param-buffer.md`](docs/re/14-brun-unlock-param-buffer.md) | 參數傳遞機制 | `ds:74h` 是參數累積指標;`BRUN30` 補齊到 44% |
