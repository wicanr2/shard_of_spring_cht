...
| 項目 | 卡在哪 |
|---|---|
| **技術棧定案** | ⛔ **這一項卡住 remake 的第一行程式碼**。`docs/spec/03-engine-plan.md` §1 列了建議與三個替代方案,**尚未定案**,由專案負責人決定 |
| **DOSBox oracle** | `tools/dosbox_run.sh` 未建。開機的手冊查詢題目由負責人提供(見 §5「不碰的東西」)|
| **模組內字串的中文化** | 1,012 段未動,卡在「檔案位移 ↔ DS 位址」未解 → 不知道每條字串由誰在哪個畫面顯示(`translations/README.md` §1)|
...
**現在卡住的不是 RE,是兩個決定**:①`docs/spec/03-engine-plan.md` §1 的技術棧沒定案,
在那之前引擎程式碼一行都不能寫;② DOSBox oracle 還沒建起來,
而 `docs/spec/00-index.md` 列的五項「實作前必讀」裡有一項需要實跑裁決。
