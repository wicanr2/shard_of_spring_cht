...
| 三張派工表的位置與內容 | `docs/re/07`,語意未解(阻擋在程式碼／資料分離)|

### 進行中

| 項目 | 卡在哪 |
|---|---|
| **程式碼／資料分離** | 線性掃描分不出兩者,已證實在資料區造出假的 `int` 指令(`docs/re/07` §3 有硬證據)。要改成從進入點做流程追蹤 |
| **解出三張派工表的語意** | 表位址已確認,但呼叫統計含雜訊,阻擋在上一項 |
...
下一個瓶頸是**程式碼與資料的分離**:目前的線性掃描分不出兩者,
已證實在資料區造出假的 `int` 指令,因此派工表的使用統計還不能用。
做法是改成從 `bz+0x30` 做流程追蹤(recursive descent),
追完重跑統計、索引越界問題應自動消失 —— 那就是驗證。
...
| [`07-dispatch-tables.md`](docs/re/07-dispatch-tables.md) | 派工表 | 三張表已取出;**統計含假陽性**,要先分離程式碼與資料 |
