...
| **打獵量到一次 +3** | `docs/re/176`,用 `tools/oracle_patch.py` 給第一位隊員 Hunting(只動 `workplace/` 複本),補給品 **20 → 23**。⚠ 一個成功樣本推不出面數;多數次數是失敗。順帶兩個 oracle 事實:**營地動作不寫檔**(每次啟動狀態相同、不能 save-diff)、**RNG 跟著時間走**(timeline 裡塞不同的 `wait:` 就能取樣不同亂數) |
