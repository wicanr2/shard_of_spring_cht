...
| **還開著的** | 兩項要新 RE(遭遇的隻數、怪物施法的投入與目標格,兩項都標在遊戲畫面上)、一批版面差異、以及場景架構重構 C2(它的[重啟判準量過了、未達成](docs/spec/14-remake-worklist.md))|
...
| 怪物施法 | 施不施、放哪一招都解了,但**投入幾點**與**目標格**沒讀到 → 怪物只移動與普攻,並在畫面上標出來 |
