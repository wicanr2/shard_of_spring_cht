...
| [`docs/re/`](docs/re/) | 228 篇分析筆記,編號即閱讀順序 |
...
| 階段 | 狀態 |
|---|---|
| **逆向工程** | ✅ 結束。[`CLAUDE.md`](CLAUDE.md) §2.2 看板的**十二個子系統全部 RE-DONE**,規格標 READY。`docs/re/` **228 篇**筆記 |
...
| **音效** | **部分** | 七個代碼的波形全部照抄,**觸發點只接四個**(命中、劈砍、死亡、施法)。`BP` 的九個呼叫端沒有逐條讀、`FL` 的語意沒讀、`BR` 在 `CMBT` 裡沒有呼叫端 —— ⛔ 不猜位置硬接 |
...
| 項目 | 為什麼還開著 |
|---|---|
| 音效的 `BP` / `FL` | 呼叫端的語意要一個一個讀。**接錯位置玩家聽不出來**,所以寧可不接 |
