...
| 世界地圖 | `C` 紮營、`P` 隊伍資訊、`N` 名冊、`S` 存檔、`A` 另存、`Q` 離開回主選單 |
| 地城 | `C` 紮營、`S` 存檔、`A` 另存、`Q` 離開回主選單(原版的 `Q` 是離開遊戲)|
...
⚠ 按 `Q` 之後會再問一次「要儲存這場遊戲嗎」—— 原版就是問兩句,
答完才離開。

### 商店賣出（原版沒有的）

原版的商店只能買。這個重製版讓你**賣東西換錢**，在商店裡按 **F2**：

```
F2 → 選是誰的東西（數字） → 選背包哪一格（字母）
```

- 收購價是**那間店售價的 65%**，所以物價高的城鎮賣得多一點
- 每間店**只收自己賣的那一類** —— 劍舖不收藥水
- **裝備中的要先卸下**才能賣
- 任務道具賣不掉（不在任何店的收購範圍內）

ESC 是兩層：先退回選人，再按一次離開賣出。

⚠ 這是**重製版加的**，1986 年的原版沒有這個功能。畫面上也會標出來。

### 音樂與音效

原版全遊戲只有兩首曲子:通關曲與死亡曲。世界地圖、地城、城鎮**沒有背景音樂** ——
1986 年的 PC 喇叭遊戲多半如此。

但**戰鬥有音效**:打中、劈砍、倒下、施法各有一個聲音,都是原版讀出來的
頻率與長度,用同一個 PC 喇叭方波產生。
...
戰鬥中的 `S` 是**原版的音效開關**,與 F5 是兩回事:F5 選的是「放哪一種音樂」,
`S` 是「要不要有聲音」。原版那個鍵管的就是音效。
