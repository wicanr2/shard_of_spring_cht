...
| 74–83 | 10 | 字元 | **背包十格的「已辨識」旗標**(`'1'` = 已辨識)—— 不是背包的後五格。⚠ 原版四個寫入端裡有**一個差一格**([`re/168`](../re/168-loot-pickup-and-an-off-by-one.md))| 已確認([`re/167`](../re/167-record-field-accessor-and-identified-flags.md) §2)|
| 86 | 1 | 字元 | **今天已經用過技能**(`'1'` = 用過)。`H)unt` 與 `I)dentify` 共用。⚠ 出貨存檔是**空白**不是 `'0'` | 已確認([`re/166`](../re/166-camp-hunt-identify-print.md) §1)|
