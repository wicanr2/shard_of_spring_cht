...
**有兩組**,由各模組啟動時寫進 `0x3D8` 的那一個 byte 決定
([`re/146`](../re/146-maze-exit-negatives-and-palettes.md) §4):

| `0x3D8` | 誰寫的 | 色(取自原版實跑截圖)|
|---|---|---|
| **`0x0A`** | `WRLDMOVE` | 黑 / 綠 `00AA00` / 紅 `AA0000` / 棕 `AA5500` |
| `0x0E` | `MAZEMOVE`、`CMBT`、`MENU`、`MTEST` | 黑 / 青 `00AAAA` / 洋紅 `AA00AA` / 白 `AAAAAA` |

`TOWN` / `CAMP` / `CHARUTIL` / `START` **沒有寫** —— 沿用前一個模組設的。

⚠ 本節原本只寫 `0x3D8 = 0x0E` 一組。那個值是對的,**但它只讀了一個模組** ——
用錯調色盤畫出來的地圖仍然是一張地圖,只是顏色不對。

`0x3D9`(顏色選擇)**從來沒有被任何模組或 `BRUN30` 寫過**,
所以亮度位元維持 BIOS 設模式時的狀態。
