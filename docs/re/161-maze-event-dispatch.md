...
| 5 | 命中的事件**會被就地作廢**(欄 0/1/2 覆寫成 99),前提是**剛從 `CMBT` 回來** —— 打完仗才作廢 | **已確認**(前提見 [`165`](165-dgroup-string-map.md) §3)|
...
011576  INT 3F:62  字串比較          ; 條件 B:ds:3798 == "CMBT"(docs/re/165 §2)
...
**條件 B 已解**([`165`](165-dgroup-string-map.md) §3):`ds:3798` 是模組代號,
比較的對象是 `"CMBT"` —— 所以作廢只發生在**打完一場由事件引發的戰鬥、回到迷宮時**。
房間敘述不引發戰鬥,因此**不會被作廢,可以重複看**。
...
| 項目 | 狀態 |
|---|---|
| ~~`0x11576` 的字串比較~~ | **已解**:`ds:3798 == "CMBT"`([`165`](165-dgroup-string-map.md) §2/§3)|
| 作廢是否跨存檔保留 | **未解** |
