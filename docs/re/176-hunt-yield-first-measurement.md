...
同一條 timeline 跑兩次結果不同,而**唯一的差別是中間多了一次截圖**(多花幾秒)。
所以要取樣不同的亂數,**在 timeline 裡塞不同長度的 `wait:` 就夠了**,
不必改任何遊戲狀態。

⚠ 反過來說:**同一條 timeline 不保證重現同一個結果**。
`tools/dosbox_run.sh` 的 `cycles=fixed 4000` 讓速度可重現,但**亂數不可重現**。
...
| 項目 | 狀態 |
|---|---|
| 打獵的擲骰面數與加項 | **未解**(§2)—— 要跑幾十次取分佈 |
| 補給品的上限 `ds:6F10` | **未量** —— 要先把補給品堆到上限 |
| 打獵的成功率 | **未量** |
