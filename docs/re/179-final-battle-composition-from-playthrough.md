...
| # | 結果 | 信心 |
|---|---|---|
| 1 | 目標 **533** = `Siriadne !` + **2 隻 `Great Dragon`** | **已確認** —— 本篇是第 3 級,[`180`](180-scripted-fight-monster-list.md) 的反組譯獨立得到同一答案 |
...
⚠ **這一篇不是反組譯結論**,是第 3 級證據。它的用途是**當反組譯那一輪的獨立對照**。

✅ **對照做完了,兩條鏈的答案相同**([`180`](180-scripted-fight-monster-list.md)):
`MAZEMOVE 0x12BE4` 往 `ds:372C/372E/3730` 寫的正是 **53 / 53 / 71**。
兩條鏈沒有共用前提 —— 一條看玩家在螢幕上遇到什麼,一條看程式往哪個位址寫了什麼數字
—— 所以第 1 項的信心升到**已確認**(`CLAUDE.md` §2.1 條件 3)。
...
| 項目 | 狀態 |
|---|---|
| ~~腳本指定怪物的機制~~ | **已解**:`ds:372C + 2i`,哨兵 99([`180`](180-scripted-fight-monster-list.md))|
| 三隻的站位與初始朝向 | **未解** |
