...
017C6C  mov di, 977Eh / INT 3F:91   ; × 面數 = 100
017C74  mov di, 9464h / INT 3F:81   ; + 1
017C7F  mov bx, 1Ah / INT 3D:03     ; INT()  ← 0x1A 是累加器位址,不是面數
...
**分母是 100,欄6 就是百分比**([`157`](157-magic-item-rate-is-a-percentage.md)),所以:

- `100` = **必定成立**(藥水、火把 —— 用了一定有效)
- `0` = **永不成立**(鑰匙、任務物品 —— 沒有魔法效果)
- 7–15 = **7%–15%**(戒指法杖 —— 大多數時候會失敗)

⚠ 這一節曾經把分母寫成 26,因而算出「約 27%–58%」。
兩個端點(100 / 0)在**任何**分母下都給出同樣的結論,所以它們驗不出尺度 ——
要驗尺度得挑中段的樣本。
