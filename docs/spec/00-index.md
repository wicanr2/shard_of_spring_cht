...
| 洞 | 出處 | 現在怎麼辦 |
|---|---|---|
| **戰場地形** | [`re/227`](../re/227-fastcmbt-is-nine-get-arrays.md) §2/§3 | 九張圖的身分已定案(= `EXITSPOT`/`BORDER`/`WATER`/`LAVA`/`MAZEWALL`/`DOOR` ＋ 三張暴風的打包本),但**地形從哪來還沒裁決** —— 合成地形實測與產生器的讀法矛盾。下一步是讀 `ds:3740` 的填入端。引擎**不畫地形** |
