...
| **M5** | 迷宮場景 + 事件 | ✅ **完成**:六個 `.SQZ` 走得動、能見度裁視野、事件三類(文字/傳送/跨關卡)、`MAZEITEM` 29 個圖塊;離開迷宮 = **走到格陣列界外**([`re/147`](../re/147-leaving-the-maze.md));寶石謎題與治療池的規則已實作,觸發點在 [`re/161`](../re/161-maze-event-dispatch.md) 解掉了。⚠ 兩軸的對應到 2026-08-18 才校正([`re/224`](../re/224-maze-axes-major-is-north-south.md))—— 在那之前治療池**沒有路徑可以觸發** |
| **M6** | 法術與道具 | ✅ **完成**:33/33 有分派(4 個標「未解」不套用效果)、三個施法閘門、狀態強度、魔法道具發動;投入點數由玩家輸入(`castSPState`)|
...
| **M10** | 戰場:格陣列 31 寬、行動點數、離場 | ✅ **完成**:點數 = 速度、前進 2 / 轉向 1 / 攻擊 3、只打面前那一格、走上外圈離場;隊伍佈陣每列三個、基準 (13,13)([`re/160`](../re/160-party-deploys-three-per-row.md)、[`re/164`](../re/164-board-stride-31-and-base-13.md));戰場高度 = **31**([`re/218`](../re/218-four-named-assumptions-audited.md) §2);怪物 AI 的出場、鎖定、逐步選格、**施法**都解了並接線(投入 = 單價 ×2、目標格 = 鎖定的那個人,[`re/226`](../re/226-monster-cast-invest-and-target.md))|
