...
| **M5** | 迷宮場景 + 事件 | ✅ **完成**:六個 `.SQZ` 走得動、能見度裁視野、事件三類(文字/傳送/跨關卡)、`MAZEITEM` 29 個圖塊;⚠ 離開迷宮的原版做法未解,暫用 ESC |
| **M6** | 法術與道具 | ✅ **完成**:33/33 有分派(4 個標「未解」不套用效果)、三個施法閘門、狀態強度、魔法道具發動;⚠ 投入點數固定投一級,原版讓玩家輸入 |
| **M8** | 城鎮 / 商店 / 營地 / 名冊 | ✅ **完成**:建築清單、商店買入(售價與**原版實跑的價目表**逐項相符)、分頁、旅店住宿、酒館買食糧與傳聞、**訓練所升級**、**治療所四項**、營地睡覺/休息、名冊(入隊/離隊/刪除,寫回逐位元組驗證);⚠ 賣出 —— **原版沒有這個功能**([`re/142`](../re/142-town-service-prices.md) §4)|
| **M10** | 戰場:15×15、行動點數、離場 | ✅ **完成**:點數 = 速度、前進 2 / 轉向 1 / 攻擊 3、只打面前那一格、走上外圈離場;⚠ 初始佈陣與怪物 AI 是佔位([`12-combat-board.md`](12-combat-board.md))|
