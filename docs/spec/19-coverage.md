...
- **`KEYPAD TEMPLATE`(CMBT:31)與周邊的方塊字元**(索引 32–40):原版是小鍵盤
  方向對照圖。**佈局在 2026-08-18 的 QA 實跑拍到了**:
  ...
  ⚠ 兩張圖各自解掉一個問題:戰鬥那張的 `?` 說明檢視面板綁的是 `?` 不是 `/`;
  **戰鬥那張沒有逃跑鍵**,印證 [`re/103`](../re/103-flee-is-leaving-the-field.md)
  的「逃跑不是指令」。⚠ 地城那張的 `Q` 對應什麼**沒查**(實跑按下去沒有可見反應)。
  引擎的按鍵表(`shell_scene.go`)仍是條列不是九宮格 —— 要做視覺對照圖現在有素材了。
