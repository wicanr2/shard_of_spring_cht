...
| **`CHARUTIL` 的程式碼** | 兩個輸入點都設 `ds:6808 = 9`([`re/199`](../re/199-name-limit-is-nine.md))|

**上限是 9**,而 `CHARS.DAT` 位移 2–11 的欄位是 10 bytes ——
輸入讀完之後才補到 10 個字元。兩個數字都對,只是講的不是同一件事。
`town.NameMaxRunes = 9`。

> **判準**:兩份原版資料打架時,**去讀強制那件事的那一行**,
> 不要在兩個說法之間投票。
...
⛔ **未接不等於沒做完**,但**未接又不說原因**就是沒做完。
`tools/check_module_text.py` 擋這件事:`wired` 空著的列,`note` 欄必須非空,
否則整支回非零。理由是那樣的一列與「還沒輪到它」長得一模一樣 ——
下一個人得把整段重查一遍才知道它是漏掉的還是決定不接的。

目前 33 段未接。原因分四類(**每一段的 `note` 欄寫的是它自己的那一條**,
⛔ 這裡不列各類的段數 —— 分類靠關鍵字數出來的數字對不齊,
而一個對不齊的數字比沒有數字更糟):

| 類 | 意思 |
|---|---|
| 規則沒讀出來 | 引擎沒有那個功能,而且**照 [`CLAUDE.md`](../../CLAUDE.md) §2 的閘門,補之前要先回 RE**(缺口列在 [`19-coverage.md`](19-coverage.md) §2)|
| 原版有兩份、引擎一份 | 同一句話在兩個模組措辭或標點不同;已接到另一份那一列 |
| 版面對不起來 | 定寬表頭、3×3 字母格、句中片段這一類 |
| 已裁定不接 | 謎題答案要照原文拼寫、沒有讀取畫面、原版的格子圖與表頭靠空白對齊 |
