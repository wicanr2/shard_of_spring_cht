...
var Unresolved = []string{
	// 原版一場遭遇是**一群**,而且會混種(docs/re/225:世界地圖 7 隻地精、
	// 拉利斯 5 隻三種不死系)。算式的位置找到了(`CMBT 0x11180`,隻數落在
	// `ds:943A`),但 `INT 3F:C4` 的運算元約定沒解,所以**不知道它讀的是哪一欄**。
	// ⛔ 在解開之前維持一場一隻,不湊一個看起來合理的數字。
	"原版一場遭遇是一群怪(而且會混種),隻數的算式未解 —— 這裡一場只放一隻",
	// 施不施法(rules.MonsterCasts)與放哪一招(rules.MonsterSpell)都解出來了,
	// 但**投入幾點**與**放在哪一格**沒有讀到(docs/re/186 §4、170)——
	// 群體傷害是 5×5,中心點在哪裡決定它打到誰。
	// ⛔ 兩個都湊一個值就等於自己發明怪物的 AI,所以整條先不接。
	"怪物施法的投入點數與目標格未解 —— 這裡的怪物只移動與普攻",
}
