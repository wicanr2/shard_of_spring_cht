...
var (
	...
	// ⚠ 高度 108 是**兩行 24 px 字 + 內距**算出來的:
	// 指令提示一行 + 未解說明一行。先前是 84,在 20 px 字下剛好兩行,
	// 換成倚天 24 px 之後第二行就掉到框外(畫得出來,但壓在畫面邊緣)。
	// 底緣 644+108 = 752,離畫布底 16 px = Margin,沒有溢出。
	Prompt = Rect{Margin, View.Bottom() + Margin, ScreenW - 2*Margin, 108}
