...
//
// **已確認**(docs/re/213):`MAZEMOVE 0x1078C` 起把繪製邊界算成
// 「中心 − ds:35B2」與「中心 + ds:35B2」,兩軸各取一次 —— 那就是
// 切比雪夫半徑。`USERLIB` 的置中偏移 `(4 − 能見度) × 17` 是獨立佐證:
// 那個算式只有在「滿視野半徑 4、實際半徑 = 能見度」時才成立。
//
// ⚠ 常數留著不是因為還有疑問,是因為**世界地圖那一邊用同一條規則**
// (半徑取天色),兩處要一起改。
...
// LootEvents 是「走到這一格會拿到一件道具」的六個事件(docs/re/202)。
//
// 派工是兩層 `ON … GOTO`:先 `INT(事件編號 ÷ 100)` 選迷宮,
// 再 `事件編號 − 100 × 迷宮` 選那一關的第幾個事件。
//
// ⚠ **`TargetRiddle`(705)不在這裡** —— 它的道具是答對謎題的獎賞,
// 由 `riddleKey` 給,不是踩到格子就給。
var LootEvents = map[int]int{
	202: 49, // 鑰匙     ——「你在房間角落發現一把小鑰匙。」
	303: 48, // 油燈     ——「某人的臥房。」
	304: 50, // 銅鑰匙   ——「一間小房間的角落。」
	305: 6,  // 戰斧     ——「一人的戰斧插在頭骨上……」
	314: 52, // 金鑰匙   ——「空房間。」
...
// RiddleReward 是 Eldron 謎題答對的獎賞:**風暴戒**(`MAZEMOVE 0x13042`)。
const RiddleReward = 29
