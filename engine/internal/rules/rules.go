...
// expBase 是經驗曲線的底數。`TOWN.EXE` 的 `ds:715E`,MBF 單精度 1.8
// (docs/re/223)。
//
// ⚠ 型別是 **float32** 不是 float64 —— 原版算的是 MBF 單精度,
// 而 `float32(1.8)` 與 MBF 的 1.8 位元相同。用 float64 在第 11 級起會差 1–2 點。
const expBase = float32(1.8)

// expScale 是 `TOWN.EXE` 的 `ds:6F5C`,MBF 100.0(docs/re/223)。
const expScale = float32(100)

// ExpForLevel 回傳從第 n 級升到第 n+1 級所需的**累計**經驗。
//
//	門檻(n) = INT( (1.8^n + 2n) × 100 )
//
// 出自 `TOWN.EXE 0x10FF4`–`0x11047`(docs/re/223),**不是手冊 p.47 那張表**。
//
// ⚠ **手冊那張表是這條公式截到百位的印刷版**,二十列一列不差 ——
// 而截尾在低等級差很多:第 1 級真正要 **380**,手冊寫 300。
// 照手冊實作的話玩家每一級都比原版早升,而畫面上沒有任何數字會顯示成錯的。
// (2026-08-18 原版實跑量到 380 / 724 / 1183 / 1849,四點與公式完全吻合。)
//
// ⚠ 最後的取整是 `INT()`(截尾)不是四捨五入 —— 第 4 級 1849.76 → **1849**。
func ExpForLevel(level int) int {
	p := float32(1)
	for i := 0; i < level; i++ {
		p *= expBase
	}
	return int((p + float32(2*level)) * expScale)
}

// MaxLevel 是升級的上限。⚠ **公式本身沒有上限**(docs/re/223)——
// 20 來自手冊 p.47 那張表的長度,原版有沒有這道閘門**沒讀**。
const MaxLevel = 20
