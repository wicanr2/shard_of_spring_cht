...
// CampUnresolved 是營地畫面的註記。
//
// ⚠ 這一條**不是「變數未解」**。`ds:3534` 早就定案了:它是**當前迷宮編號**,
// `99` = 不在任何迷宮(docs/re/169 §4 已確認,docs/re/174 實跑驗過)。
// 原版打獵的閘門因此是 `ds:3534 ≥ 99` —— **只問「在不在迷宮」**。
//
// 本引擎多分了一層:城鎮裡的營地也算室內(`townState.wild`)。
// 那是**引擎的決定**,不是讀出來的規則 —— 照原版的閘門,
// 在城鎮營地按 H 是打得到獵的。留這條註記是要讓玩家知道
// 「這裡打不到獵」是我們加的,不是原版這樣。
var CampUnresolved = []string{
	"原版打獵的閘門只問「不在迷宮」;本引擎另外把城鎮裡的營地也算室內",
}
