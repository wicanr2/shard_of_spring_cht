...
⛔ **「381/381 已接」不等於「381 段都會出現在畫面上」。** 一個沒有人引用的
`const` 照樣通過這裡的比對(Go 不會警告未使用的常數),而症狀與「接好了」
一模一樣:什麼都不會壞,那句話只是永遠不出現。所以**接線的那一輪要自己確認
常數有呼叫端**,不要拿這支工具的綠燈當作已經上畫面的證據。
(2026-08-17 逐一核過最後十段:三行陣型格、`MsgPts`、三個名冊表頭、
兩個提問、`mazeOneMoment`、治療池那一句,全部有實際的繪製/訊息呼叫端。)
